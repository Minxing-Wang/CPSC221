...
Node *pred = target->left;
        ...
        target->key = pred->key;
        delete_node(target->left, pred->key);
...
bool allLessThan(Node* root, int val) {
  if (root==NULL) return true;
  return (root->key < val && allLessThan(root->left, val) && 
allLessThan(root->right, val));
}

bool allGreaterThan(Node* root, int val) {
  if (root==NULL) return true;
  return (root->key > val && allGreaterThan(root->left, val) && allGreaterThan(root->right, val));
}
...
bool is_bst(Node* root) {
  if (root==NULL) return true;
  return allLessThan(root->left, root->key) && 
allGreaterThan(root->right, root->key) && is_bst(root->left) && 
is_bst(root->right);
}
